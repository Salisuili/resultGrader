      ...
      if ($size > 0) {
        $resourse = fopen($file, "r");
          while ($getData = fgetcsv($resourse,1000,",")) {
            if (is_array($getData) && count($getData) == 5) {
             $sql ="insert into student(fname,mname,lname,sex,admissionNo) values('$getData[0]','$getData[1]','$getData[2]','$getData[3]','$getData[4]')";
        $result = query($sql);
        if($result){
          $error = "students added successfully";
          }else{
            $error ="fail to upload students. contact 08101418633";
          }
        }else{
        $error = "invalid file";
      }
...
?>
   ...
   <div class=" container row">
       ...
       <div class="col s4 container green-text text-darken-5 c">
           <form action="students-registration.php" method="post" class="" enctype="multipart/form-data">
            <div class="round">
        ...
        <center>
          <button class="btn-large green z-depth-0" type="submit" name="submit">register&nbsp;students</button>
        </center>
        </div>
        </form>
       </div>
